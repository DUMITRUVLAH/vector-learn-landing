/**
 * Server copy of src/lib/par/payeeTypeDetector.ts (two-copies rule — the server
 * must not import client code). Keep byte-identical with the client version.
 *
...
const COMPANY_KEYWORDS_RE = /\b(BC|SA|S\.A\.|SRL|S\.R\.L\.|GmbH|LLC|Ltd|Corp|ONG|AO|PÂ|Banca|Bank|Credit|Asigur|Companiei|Compania|Group|Holdings|Institut|Fundatia|Fundația|Asociatia|Asociația|Asociatiei|Organizatia|Organizația|Întreprinderea|Intreprinderea|Individual[ăa]|Individuala|ANSP|IMSP|CNA|ANP)\b|ÎI|Î\.I\.|ÎS|Î\.S\.|ООО|ОАО|ЗАО|АО/;
...
export function detectPayeeType(name: string): PayeeType | null {
  ...
  if (
    words.length >= 2 &&
    words.every((w) => /^[A-ZĂÂÎȘȚÉÈÇÖÜÄА-ЯЁ][a-zA-ZăâîșțéèçöüÄÖÜа-яёА-ЯЁ\-']*$/.test(w)) &&
    words.every((w) => /^[A-ZĂÂÎȘȚÉÈÇÖÜÄА-ЯЁ]/.test(w))
  ) {
    return "fizic";
  }

...
// ── finLedgerEntries (simple flat ledger used by EXPORT + REVALUATION modules) ──
// Distinct from the double-entry finJournalEntries/finJournalLines above. Built by
// the EXPORT and MULTICURRENCY branches which use a flat one-row-per-posting model.
export const finLedgerEntries = pgTable(
  "fin_ledger_entries",
  {
    ...
    debitCents: bigint("debit_cents", { mode: "number" }).notNull().default(0),
    creditCents: bigint("credit_cents", { mode: "number" }).notNull().default(0),
    fxGainLossCents: bigint("fx_gain_loss_cents", { mode: "number" }).default(0),
    description: text("description"),
  ...
  (t) => ({
    fleTenantIdx: index("fle_tenant_idx").on(t.tenantId),
    fleTenantPeriodIdx: index("fle_tenant_period_idx").on(t.tenantId, t.periodMonth),
  })
);

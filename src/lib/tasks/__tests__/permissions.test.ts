...
describe('canFullyEditTask', () => {
  ...
  it('editorul boardului poate', () => {
    expect(canFullyEditTask(task(), ctx({ board: { id: BOARD, created_by: ALTUL, can_edit: true } })))
      .toBe(true);
  });

  it('creatorul boardului poate, chiar dacă nu e membru', () => {
    expect(canFullyEditTask(task(), ctx({ board: { id: BOARD, created_by: EU, can_edit: false } })))
      .toBe(true);
  });
  ...
  it('boardul necunoscut sau nepotrivit nu dă drepturi', () => {
    ...
    expect(canFullyEditTask(task(), ctx({ board: { id: 'alt-board', created_by: EU, can_edit: true } })))
      .toBe(false);
  });

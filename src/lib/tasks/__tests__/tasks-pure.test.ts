...
describe("groupTasksByDue", () => {
  ...
  it("exclude task-urile gata din toate secțiunile", () => {
    const groups = groupTasksByDue([task({ status: "done", due_date: "2026-07-01T00:00:00Z" })], today);
  ...
  it('ziua a 7-a intră încă în „săptămâna asta", a 8-a nu', () => {
    const groups = groupTasksByDue(
      [task({ id: "z7", due_date: "2026-08-08T00:00:00Z" }), task({ id: "z8", due_date: "2026-08-09T00:00:00Z" })],
      today,
    );
...
describe("filterTasks", () => {
  ...
  it("includeSubtasks / includeDone le readuc", () => {
    const ids = filterTasks(dataset, { ...base, includeSubtasks: true, includeDone: true }, today).map((t) => t.id);
  ...
  it("caută în titlu și descriere, case-insensitive", () => {
    ...
    expect(filterTasks(items, { ...base, search: "raportul" }, today).map((t) => t.id)).toEqual(["x", "y"]);
  });
  ...
  it("filtrul de etichetă compară doar label-ul, nu culoarea", () => {
    const items = [task({ id: "cu", tags: ["Urgent|red"] }), task({ id: "fara", tags: ["Altceva"] })];
    expect(filterTasks(items, { ...base, tag: "urgent" }, today).map((t) => t.id)).toEqual(["cu"]);
    expect(filterTasks(items, { ...base, tag: "Urgent|blue" }, today).map((t) => t.id)).toEqual(["cu"]);
  });
...
describe("tags", () => {
  ...
  it("distinctTags deduplică fără să țină cont de majuscule", () => {
    const tags = distinctTags([{ tags: ["Urgent|red", "Client"] }, { tags: ["urgent|red", "Client"] }, { tags: null }]);

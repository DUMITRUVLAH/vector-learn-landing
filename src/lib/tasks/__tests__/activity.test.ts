...
describe('readActivity — restul câmpurilor', () => {
  it('păstrează din ce în ce pentru status și prioritate', () => {
    expect(readActivity({ action: 'status_changed', from_value: 'todo', to_value: 'done' }))
      .toMatchObject({ key: 'status_changed', from: 'todo', to: 'done' });
    expect(readActivity({ action: 'priority_changed', from_value: 'medium', to_value: 'high' }))
      .toMatchObject({ from: 'medium', to: 'high' });
  });

  it('termenul are trei cazuri distincte: pus, mutat, scos', () => {
    expect(readActivity({ action: 'due_date_changed', from_value: null, to_value: '2026-09-18' }).key)
      .toBe('due_date_set');
    expect(readActivity({ action: 'due_date_changed', from_value: '2026-09-11', to_value: null }).key)
      .toBe('due_date_cleared');
    expect(readActivity({ action: 'due_date_changed', from_value: '2026-09-11', to_value: '2026-09-18' }).key)
  ...
  it('coloana și boardul păstrează id-urile — numele se rezolvă la randare', () => {
    expect(readActivity({ action: 'list_changed', from_value: 'l1', to_value: 'l2' }))
      .toMatchObject({ key: 'list_changed', from: 'l1', to: 'l2' });
  });
});
...
describe('groupActivityByDay', () => {
  it('grupează pe zi, păstrând ordinea primită', () => {
    ...
    expect(g.map((x) => [x.day, x.entries.length])).toEqual([['2026-09-10', 2], ['2026-09-09', 1]]);
  });

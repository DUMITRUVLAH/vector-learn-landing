...
import {
  completionTrend, computeKpis, countByStatus, dueSoon, loadByPerson, pendingApprovalsFor,
  statsByBoard,
} from '../analytics';
...
describe('buildGanttLayout', () => {
  ...
  it('datele inversate se normalizează', () => {
    const layout = buildGanttLayout(
      [task({ start_date: '2026-08-10T00:00:00Z', due_date: '2026-08-04T00:00:00Z' })],
      { padDays: 0 },
    );
  ...
  it('task-urile fără nicio dată nu se pierd, ies separat', () => {
    const layout = buildGanttLayout(
      [task({ id: 'fara' }), task({ id: 'cu', due_date: '2026-08-05T00:00:00Z' })],
      { padDays: 0 },
    );
  ...
  it('sub-taskurile nu apar pe axă', () => {
    const layout = buildGanttLayout(
      [task({ parent_task_id: 'p', due_date: '2026-08-05T00:00:00Z' })],
      { padDays: 0 },
    );
  ...
  it('marchează milestone-urile', () => {
    const layout = buildGanttLayout(
      [task({ is_milestone: true, due_date: '2026-08-05T00:00:00Z' })],
      { padDays: 0 },
    );
...
describe('statsByBoard / countByStatus / dueSoon', () => {
  it('grupează pe board și calculează procentul', () => {
    const stats = statsByBoard(
      [
        task({ board_id: 'b1', status: 'done' }),
        task({ board_id: 'b1' }),
        task({ board_id: 'b2', status: 'done' }),
      ],
      '2026-08-01',
    );
  ...
  it('countByStatus întoarce toate cheile, chiar și cele cu zero', () => {
    expect(countByStatus([task({ status: 'todo' })])).toEqual({
      todo: 1, in_progress: 0, pending: 0, done: 0,
    });
  });

...
import { DEFAULT_SORT, groupTasks, sortTasks, toggleSort, type SortState } from "../sorting";
...
describe("sortTasks", () => {
  ...
  it("ordinea pe prioritate nu depinde de limba interfeței", () => {
    ...
    const input = [task({ id: "a", priority: "medium" }), task({ id: "b", priority: "urgent" })];
  ...
  it("task-urile FĂRĂ termen rămân ultimele în AMBELE direcții", () => {
    ...
    expect(sortTasks(input, { key: "due_date", dir: "asc" }).map((t) => t.id)).toEqual(["devreme", "tarziu", "fara"]);
    // Inversând, „fără termen" NU trebuie să sară în capul listei: altfel
    // utilizatorul primește sus exact ce nu are informație.
    expect(sortTasks(input, { key: "due_date", dir: "desc" }).map((t) => t.id)).toEqual(["tarziu", "devreme", "fara"]);
  });
  ...
  it("titlurile se compară ignorând diacriticele", () => {
    const out = sortTasks([task({ id: "z", title: "Zebră" }), task({ id: "s", title: "Ștefan" })], {
      key: "title",
      dir: "asc",
    });
...
describe("groupTasks", () => {
  ...
  it('banda „fără responsabil" e ultima', () => {
    const out = groupTasks([task({ id: "gol" }), task({ id: "ana", assignees: [ANA] })], "assignee", {
      names: NAMES,
      unassignedLabel: "Fără responsabil",
    });

...
describe("maparea coloană → status", () => {
  ...
  it("maps_to_status bate numele când cele două se contrazic", () => {
    expect(moveStatusPatch("todo", { is_done_list: false, name: "În lucru", maps_to_status: "pending" }, now)).toEqual({
      status: "pending",
      completed_at: null,
    });
  });
  ...
  it("is_done_list rămâne autoritar peste maps_to_status", () => {
    expect(moveStatusPatch("todo", { is_done_list: true, name: "Готово", maps_to_status: "todo" }, now)).toEqual({
      status: "done",
      completed_at: now(),
    });
  });
...
describe("traducerea erorilor din DB", () => {
  ...
  it("recunoaște gardul de aprobare", () => {
    expect(
      taskErrorMessage({ code: "42501", message: "Task-ul are nevoie de aprobarea unei persoane autorizate" }, t),
    ).toBe("board.errors.needsApproval");
  });

  it("recunoaște blocajul de dependență", () => {
    expect(taskErrorMessage({ message: "Nu poți finaliza task-ul: are blocatori nefinalizați" }, t)).toBe(
      "board.errors.blockedByDependency",
    );
  });

  it("recunoaște mesajele englezești ale RPC-urilor", () => {
    expect(taskErrorMessage({ code: "42501", message: "Only HR or managers can create boards" }, t)).toBe(
      "board.errors.boardCreateForbidden",
    );
  ...
  it("cade pe cod când mesajul nu e recunoscut", () => {
    expect(
      taskErrorMessage(
        { code: "42501", message: 'new row violates row-level security policy for table "hr_tasks"' },
        t,
      ),
    ).toBe("board.errors.forbidden");
    expect(taskErrorMessage({ code: "23514", message: "ceva neprevăzut din trigger" }, t)).toBe(
      "board.errors.invalidData",
    );
  });

...
export function TaskAccessRulesPage() {
  ...
  const groupRule = (subject: "managers" | "all_employees") => rules.find((r) => r.subject_type === subject);
  const personRules = useMemo(() => rules.filter((r) => r.subject_type === "user"), [rules]);
  ...
  return (
    <TasksLayout>
      <div className="mb-5">
        <h1 className="font-display text-2xl font-bold tracking-tight">{t("board.access.title")}</h1>
      ...
      <div className="mb-4 flex items-start gap-2.5 rounded-2xl border bg-muted/40 p-3.5">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">{t("board.access.explainer")}</p>
      </div>
            ...
            <CardContent className="space-y-4 p-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{t("board.access.managersHint")}</p>
                </div>
                {scopeSelect((groupRule("managers")?.scope as VisibilityScope) ?? "team", (scope) =>
                  saveGroup("managers", scope),
                )}
              </div>
              ...
              <div className="flex flex-col gap-2 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{t("board.access.employeesHint")}</p>
                </div>
                {scopeSelect((groupRule("all_employees")?.scope as VisibilityScope) ?? "own", (scope) =>
                  saveGroup("all_employees", scope),
                )}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl">
            <CardContent className="space-y-3 p-4">
              <div>
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">{t("board.access.exceptionsHint")}</p>
              </div>

              <div className="flex flex-col gap-2 rounded-xl border p-3 sm:flex-row sm:items-end">
                <div className="flex-1 space-y-1.5">
                  <Label className="block text-xs text-muted-foreground">{t("board.access.person")}</Label>
                  <AssigneePicker mode="single" value={personIds} onChange={setPersonIds} />
                </div>
                <div className="space-y-1.5">
                  <Label className="block text-xs text-muted-foreground">{t("board.access.scope")}</Label>
              ...
              {personRules.length === 0 ? (
                <p className="py-2 text-xs text-muted-foreground">{t("board.access.noExceptions")}</p>
              ) : (

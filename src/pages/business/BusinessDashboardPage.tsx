    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          {data?.findesk && (
            <>
              <StatRow
                ...
                value={formatCents(data.findesk.totalExpensesCents, "MDL")}
                trend="down"
                valueClass="text-red-600 dark:text-red-400"
              />
              <StatRow
                label="Facturi emise"
                value={formatCents(data.findesk.totalInvoicesCents, "MDL")}
                trend="up"
        ...
        <KpiCard
          ...
          icon={ClipboardList}
          loading={loading}
          error={data?.par === null && !loading}
        >
          {data?.par && (
            <>
              <div className="flex items-end gap-2 mb-3">
                <span className="text-3xl font-bold text-foreground tabular-nums">
                  {data.par.pendingCount}
                </span>
              ...
              {data.par.pendingCount > 0 && (
                <StatRow
                  label="Valoare totală pending"
                  value={formatCents(data.par.pendingValueCents, "MDL")}
                  trend="neutral"
                />
              )}
              {data.par.pendingCount === 0 && (
        ...
        <KpiCard
          ...
          icon={Building2}
          loading={loading}
          error={data?.itpark === null && !loading}
        >
          {data?.itpark && (
            <>
              <div className="flex items-end gap-2 mb-3">
                <span className="text-3xl font-bold text-foreground tabular-nums">
                  {data.itpark.activeCount}
                </span>
                <span className="text-sm text-muted-foreground mb-0.5">
                  rezidenți activi
                </span>

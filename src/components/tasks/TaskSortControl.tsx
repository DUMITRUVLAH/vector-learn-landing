...
import { DEFAULT_SORT, GROUP_KEYS, SORT_KEYS, type GroupKey, type SortKey, type SortState } from "@/lib/tasks/sorting";
    ...
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <span className="hidden sm:inline">{isSorted ? t(`sort.${sort.key}`) : t("sort.label")}</span>
      ...
      <PopoverContent className="z-[80] w-64 p-3" align="end">
        <div className="space-y-3">
          <div className="space-y-1.5">
            <Label className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("sort.label")}</Label>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      activeKey ? "bg-primary text-primary-foreground" : "hover:bg-accent/10 hover:text-primary",
                    )}
                  >
          ...
          {onGroupChange && (
            <div className="space-y-1.5 border-t pt-3">
              <Label className="text-[11px] uppercase tracking-wide text-muted-foreground">{t("group.label")}</Label>
              <div className="grid grid-cols-2 gap-1">
                {GROUP_KEYS.map((key) => (
                  <button
                    key={key}
                    ...
                    className={cn(
                      ...
                      group === key ? "bg-primary text-primary-foreground" : "hover:bg-accent/10 hover:text-primary",
                    )}
                  >
                    {t(`group.${key}`)}
                  </button>
              ...
              {isGrouped && (
                <p className="pt-0.5 text-[10px] leading-snug text-muted-foreground">{t("group.sharedHint")}</p>
              )}
            </div>
          )}
        </div>

...
export function BoardCalendarView({
  ...
  const [createBoardId, setCreateBoardId] = useState(() => boardId ?? readDefaultBoard() ?? PERSONAL);
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col gap-4 lg:flex-row">
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm">
            <div
              className={cn("grid border-b border-border/60 bg-muted/40", hideWeekend ? "grid-cols-5" : "grid-cols-7")}
            >
              {weekdays.map((day, i) => (
                <div
                  key={`${day}-${i}`}
        ...
        <CalendarRail
          iso={railIso}
          label={
            selectedIso ? format(parseISO(selectedIso), "EEEE, d MMM", { locale }) : t("board.calendar.unscheduled")
          }
        >
          {selectedIso ? (
            <>
              ...
              {canEdit &&
                ...
                (addingDay === selectedIso ? (
                  ...
                  <button
                    ...
                    className="mt-2 flex w-full items-center gap-1.5 rounded-md px-1 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-accent/10 hover:text-foreground"
                  >
                    <Plus className="h-3.5 w-3.5" />
                    {t("board.card.add")}
                  </button>
      ...
      <DragOverlay dropAnimation={null}>
        {draggedTask && (
          <div className="rounded-md border bg-card px-2 py-1.5 text-xs shadow-lg">{draggedTask.title}</div>
        )}
      </DragOverlay>
    </DndContext>
  );
...
function CalendarCell({ iso, outside, selected, hideWeekend, onSelect, children }: CalendarCellProps) {
...
function CalendarRail({ iso, label, children }: CalendarRailProps) {
...
function CalendarChip({ task, boardId, canEdit, overdue, onOpen, onToggleDone, boardName, big }: CalendarChipProps) {
  const { t } = useTasksT();
    ...
    <div className="group/chip relative">
      ...
      */}
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              {/* Ca în referință, textul trece pe rândul următor în loc să fie
                  ...
                  mik…" nu spune nimic. */}
              <span className={cn("block whitespace-normal break-words", isDone && "line-through")}>{task.title}</span>
              {boardName && (
      ...
      <button
        ...
        aria-label={
          isDone
            ? t("board.calendar.reopen", { title: task.title })
            : t("board.calendar.markDone", { title: task.title })
        }
        onClick={(e) => {
          e.stopPropagation();
          onToggleDone();
 ...
 */
function QuickEdit({ task, boardId, canEdit, onOpen }: QuickEditProps) {
  const { t } = useTasksT();
  return (
 ...
 */
function GhostChip({ task, iso, onMaterialize, pending }: GhostChipProps) {
  const { t } = useTasksT();

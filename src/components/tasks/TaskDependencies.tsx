...
export function TaskDependencies({ task, candidates, canEdit, onOpenTask, inDialog = false }: TaskDependenciesProps) {
    ...
    <div key={dep.id} className="group flex items-center gap-2 text-sm">
      ...
      {canEdit && (
        <button
          ...
          aria-label={t('board.deps.remove', { defaultValue: 'Elimină dependența' })}
        >
  ...
  return (
    <div className="space-y-1.5">
      {blockers.length === 0 && (
        <p className="text-xs leading-relaxed text-muted-foreground">{t('board.deps.none')}</p>
      )}

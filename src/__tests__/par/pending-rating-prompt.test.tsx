 ...
 *
 * Regresia raportată de owner (2026-08-31): „la finflow mereu mă întreabă de la ultimul PAR când
 * fac refresh, trebuie să fie doar o dată."
 *
 * Testul face exact ce face omul: deschide tabloul, închide popup-ul cu X (nu cu „Mai târziu"),
 * apoi „dă refresh" (remontează componenta). Pe codul vechi, unde urma se scria doar la apăsarea
 * pe „Mai târziu", a doua montare redeschidea același dialog — deci acest test pică.
 */
...
describe("PendingRatingPrompt", () => {
  it("întreabă o singură dată, chiar dacă dialogul e închis cu X și pagina e reîncărcată", async () => {
    ...
    // „Refresh": componenta se montează din nou și reinterroghează serverul.
    render(<PendingRatingPrompt />);
    await waitFor(() => expect(api.listPendingRatings).toHaveBeenCalledTimes(2));
    expect(screen.queryByRole("dialog")).not.toBeInTheDocument();
  });

  it("nu sare imediat la următoarea cerere neevaluată — cel mult o întrebare pe zi", async () => {
    const first = render(<PendingRatingPrompt />);
    expect(await screen.findByRole("dialog")).toBeInTheDocument();
